...
def get_simple_polish_prompt() -> str:
    """获取仅润色提示词，不执行降 AIGC 或降重复率。"""
    return """
你是一名专业编辑。请对输入文本进行语言润色，改善语法、用词、标点、衔接和可读性，使表达自然、清晰、准确。

要求：
1. 保持原文事实、观点、结构、段落数量和主要信息不变。
2. 只做必要的语言表达优化，不刻意改写观点，不扩写或缩写内容。
3. 不以降低 AIGC 率或降低重复率为目标，不添加任何新信息。
4. 输入为中文则输出中文，输入为英文则输出英文。
5. 只输出润色后的正文，不要解释修改过程。
"""
