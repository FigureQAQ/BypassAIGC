...
- 工作台默认使用文档上传，Word 作为首选入口。
- 处理模式统一显示为“降低 AIGC 率”“降重”和“降低 AIGC 率 + 降重”。
- 上传区域支持拖拽，模式卡片改为双列布局。
- README、会话监控、管理后台和欢迎页统一使用新术语。
...
- 请求耗时超过间隔时不再追加固定等待。
- 普通润色默认关闭高强度思考模式。
- 429、超时、连接错误和临时服务错误会自动退避重试。

## v2.8.1 配置诊断修复

- 全局 API Key 和 Base URL 会自动作为各处理阶段的回退配置。
- 示例占位 Key 不再被误判为有效配置。
- 失败任务会显示可直接处理的错误原因，并支持配置修正后断点续跑。
...
- 前端页面按路由拆包，减少首次加载脚本体积。
- Word 排版编译不再阻塞后端事件循环。
- 增加后台任务取消、定时回收和服务关闭清理。
- SSE 消息队列增加容量限制，降低慢连接的内存占用。
- 工作台增加任务停止、历史搜索、状态筛选、任务统计和工具快捷入口。
- 登录页与工作台使用全新的响应式视觉设计。
- 增加运行时管理回归测试并更新前端安全依赖。

## 当前文档能力

- 优化入口支持 Word（.docx）、PDF（.pdf）和 Markdown（.md/.markdown）上传。
- Word 与 Markdown 会优先处理摘要、正文、致谢中的普通文本段落。
